        ...
        if (data.indexOf(':') !== -1){
            switch(data.substring(0, data.indexOf(':')+1)) {
                ...
                case "SerialPort:":
                    ...
                    break;
                case "VBatt:":
                    var voltage = data.substring(data.indexOf(':') + 1).trim();
                    call("VBatt", voltage)
                    fs.appendFile('Voltage.txt', Date.now() + "," + voltage + "\n", function (err) {
                        if (err) throw err;
                    });
                    break;
                case "CBatt:":
                    ...
                    break;
                case "IBatt:":
                    var current = data.substring(data.indexOf(':') + 1).trim()
                    call("IBatt", current)
                    fs.appendFile('Current.txt', Date.now() + ":" + current + "\n", function (err) {
                        if (err) throw err;
                    });
                    break;
                default:
